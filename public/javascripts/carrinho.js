...
const removeProduto = document.querySelector(".remover-prod");

// Código principal

//Modifica o carrinho caso esteja vazio
if (containerProdutos.childElementCount === 0) {
  ...
  btnLimpaCarrinho.remove();
}

containerProdutos.addEventListener("click", async function (e) {
  if (containerProdutos.childElementCount === 0) return;
  ...
  const id_produto = Number(
    e.target.closest(".card-produto-carrinho").dataset.id
  );
  ...
  if (clicado.classList.contains("remover-prod")) {
    ...
    //Se o usuário estiver logado manda o delete para o servidor
    if (logado) {
    }
    //Caso contrário deleta do cookie
    else {
      const cartData = JSON.parse(getCookie("cr"));

      const indexTargetProduct = cartData.findIndex((p) => p.id === id_produto);
      cartData.splice(indexTargetProduct, 1);
  ...
  if(clicado.classList.contains("mais")) {
    if(logado) {

    }else {
      const cartData = JSON.parse(getCookie("cr"));

      const TargetProduct = cartData.find((p) => p.id === id_produto);
      TargetProduct.qt += 1;
  ...
  if(clicado.classList.contains("menos")) {
    if (logado) {
